    ...
    if(request('wallet') == ''){
     ...
     $maximum_withdrawal=$pkg->maximum_withdrawal ?? DB::table('packages')->where('id',$pkg->id)->first()->maximum_withdrawal ?? 100000000;
      ...
      if(request()->input('amount') < $minimum_withdrawal){
        return response()->json([
            'message' => request('wallet') == 'deposit_balance' ? 'Minimum withdrawal for Games wallet is '.Currency(Auth::guard('users')->user()->id).''.number_format($minimum_withdrawal,2).'' : 'Minimum withdrawal for '.$pkg->name.' users is '.Currency(Auth::guard('users')->user()->id).''.number_format($minimum_withdrawal,2).'',
            'status' => 'error'
        ]);
      }
